...
"""
知识星球增量抓取脚本 - 只抓取最新内容，避免重复
用于30分钟轮询场景

用法:
  ZSXQ_GROUP_ID=xxx ZSXQ_COOKIE=xxx ./venv_runner.sh tools/zsxq_fetcher_incremental.py
...
def fetch_latest(group_id: str, cookie: str) -> tuple:
    """抓取最新一页（30条）"""
    headers = {
        "Cookie": cookie,
...
def extract_topic(topic: dict) -> dict:
    ...
    return {
        ...
        "content": content[:2000] if content else "",  # 保留完整内容
        "type": topic.get("type", ""),
...
def main():
    ...
    # 抓取最新一页
    raw_topics, total = fetch_latest(GROUP_ID, ZSXQ_COOKIE)
    ...
    print(f"📊 统计: {json.dumps(result, ensure_ascii=False)}")
