...
v4.0 核心升级 (基于SKILL.md v4.0方法论):
  1. 【主涨段判定体系】N+2→N级别传导公式，启动/结束/重新焕发完整框架
  2. 【MACD稳定性分析】6种状态定义、极强/极弱操作点、零轴金叉/死叉
  3. 【X段判定体系】3条件判定、复合结构桥梁、后端确认机制
  4. 【分型/笔/线段】K线合并、特征序列法、线段破坏定理
  5. 【段数结构】3段/5段基础、盘整(1中枢)/趋势(2+中枢)
  6. 保留v3.5: 120F级别/5F套娃/双周55周线
  7. 保留v3.4: 底背离/顶背离/信号优先级/情景推演
  8. 保留v3.3: 联合区/假突破/二买二卖/传导链/双日/时间窗口
"""
...
# =====================================================
# v3.1: 55线思维 (v4.0增强 - 容错机制)
# =====================================================

def judge_55line_status(level_name, df_level, tolerance=0):
    """
    55线状态判断（含容错机制）
    
    参数:
        tolerance: 容错点数（默认0，5F/15F建议2点）
    """
    latest = df_level.iloc[-1]
    price = latest['Close']
    ma55 = latest['MA55']
    ...
    # 容错处理：差值在tolerance内视为"接近"
    diff = price - ma55
    is_above = diff > tolerance
    is_below = diff < -tolerance
    is_near = abs(diff) <= tolerance
    
    if is_near:
        status_55 = '55线临界（容错范围内）'
        key_signal = '55线胶着'
    elif is_above:
        status_55 = '55线上方'
        key_signal = '55线上方运行'
    ...
    # 结构判断（含容错）
    if is_above and macd > 0:
        structure = '主涨段'
    elif is_below and macd < 0:
        structure = '主跌段'
    elif is_near:
        # 临界状态：看MACD方向
        if macd > 0:
...
def analyze_macd_stability(df_level, level_name='30F'):
    """
    MACD稳定性分析 (v4.0核心)
    ...
    if dif >= 0 and dea <= 0 and hist > 0:
        ...
        rule = '不做空'
        win_rate = '极高'
    elif dif > dea > 0 and hist > 0:
        state = '强'
        rule = '做多胜率高'
        win_rate = '高'
    elif 0 > dif > dea and hist > 0:
        state = '中性偏强'
        rule = '观察'
        win_rate = '中'
    ...
    # 判定金叉死叉形态（简化）
    # 检查最近是否有金叉/死叉
    prev_hist = df_level['MACD_Hist'].iloc[-5:-1]
    prev_sign = np.sign(prev_hist)
    curr_sign = np.sign(hist)
    
    cross_type = '无'
    if len(prev_hist) > 0 and prev_sign.iloc[-1] < 0 and curr_sign > 0:
        cross_type = '金叉'
    elif len(prev_hist) > 0 and prev_sign.iloc[-1] > 0 and curr_sign < 0:
        cross_type = '死叉'
    
    # 判断零轴附近
    zero_axis = abs(dif) < 0.5 and abs(dea) < 0.5
    
    if cross_type == '金叉' and zero_axis:
        cross_type = '零轴金叉(极强)'
    elif cross_type == '死叉' and zero_axis:
        cross_type = '零轴死叉(极弱)'
    elif cross_type == '金叉':
        if dif > 0 and dea > 0:
            cross_type = '水上金叉'
    ...
    return {
        ...
        'description': f'{level_name} MACD{state}({rule}), DIF={dif:.2f}, DEA={dea:.2f}, 最新交叉={cross_type}'
    }


def analyze_main_trend_segment(df_level, level_name='30F', tolerance=0):
    """
    主涨段判定 (v4.0核心)
    ...
    # 容错处理
    diff = price - ma55
    is_above_55 = diff > tolerance
    is_below_55 = diff < -tolerance
    is_near_55 = abs(diff) <= tolerance
    
    is_macd_positive = macd > 0
    
    if is_above_55 and is_macd_positive:
        trend_state = '主涨段运行中'
        risk = '低'
        action = '持仓'
    elif is_above_55 and not is_macd_positive:
        trend_state = '主涨段暂停（55线上方X段）'
        risk = '中'
        action = '观察，警惕回落'
    elif is_near_55 and is_macd_positive:
        trend_state = '主涨段临界（容错范围内）'
        risk = '中'
        action = '观察能否确认突破'
    elif is_near_55 and not is_macd_positive:
        trend_state = '非主涨段临界（容错范围内）'
        risk = '中'
        action = '观望'
    elif is_below_55:
        # 检查是否在N-1级别55线或N级别中轨附近
        is_near_support = abs(price - ma55) / ma55 < 0.02
        if is_near_support and macd > 0:
            trend_state = '回踩N-1级别55线确认中'
            risk = '中'
            action = '观察能否重新焕发'
        else:
            trend_state = '非主涨段'
    ...
    return {
        ...
        'risk': risk,
        'action': action,
        'description': f'{level_name}: {trend_state}, 价格={price:.2f}, MA55={ma55:.2f}({diff:+.2f}), MACD={macd:.2f}'
    }


def analyze_segment_structure(strokes, level_name='30F'):
    """
    段数结构分析 (v4.0)
    ...
    if not strokes or len(strokes) < 3:
        return {'usable': False, 'description': '段数不足3，无法分析'}
    
    segment_count = len(strokes)
    
    if segment_count == 3:
        structure_type = '盘整（3段，1个中枢）'
        subsequent = '可能反转或延续'
    elif segment_count == 5:
        structure_type = '趋势（5段，2个中枢）'
        subsequent = '末端注意反转'
    elif segment_count >= 9:
        structure_type = '中枢扩展（9段+，更大级别）'
        subsequent = '级别升级'
    else:
        structure_type = f'复合结构（{segment_count}段）'
        subsequent = '等待方向选择'
    ...
    return {
        ...
        'description': f'{level_name}: {segment_count}段, {structure_type}, {subsequent}'
    }
...
def identify_x_segment_v40(df_level, level_name='30F', context=None):
    """
    X段判定 v4.0 (3条件)
    
    条件1: 出现在复合结构中（前段+X段+后段）
    条件2: 起点=前段最高点
    条件3: 后端确认：突破N级别55线+不创新低
    ...
    if is_above_55 and is_macd_negative:
        x_type = '55线上方X段'
        risk = '高'
        description = f'{level_name}价格>{ma55:.2f}但MACD={macd:.2f}<0，上涨中的回调/中枢震荡（X段）'
        condition3 = '待确认：需突破N级别55线+新高前不创新低'
    elif not is_above_55 and not is_macd_negative:
        x_type = '55线下方X段'
        risk = '高'
        description = f'{level_name}价格<{ma55:.2f}但MACD={macd:.2f}>0，下跌中的反弹/中枢震荡（X段）'
        condition3 = '待确认：需突破N级别55线+新高前不创新低'
...
class ChanAnalysisV40:
    ...
    def get_55line_analysis(self):
        ...
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
                tol = 2 if name in ['5F', '15F'] else 0
                result[name] = judge_55line_status(name, df, tolerance=tol)
        return result
    ...
    def generate_report(self):
        ...
        report = {
            ...
            # v4.0新增
            'macd_stability': self.get_macd_stability(),
            'main_trend_segment': self.get_main_trend_segment(),
            'segment_structure': self.get_segment_structure(),
            'x_segment_v40': self.get_x_segment_v40(),
            'segment_decomposition': {},
            'dual_day_weakness_lift': {},
        }
        return report
    
    # v3.5 新增方法
    def get_x_segment(self):
        """X段识别 (v3.5简单版，v4.0有增强版)"""
        result = {}
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
    ...
    # v4.0 新增方法
    def get_macd_stability(self):
        """MACD稳定性分析 (v4.0核心)"""
        result = {}
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
                result[name] = analyze_macd_stability(df, name)
        return result
    
    def get_main_trend_segment(self):
        """主涨段判定 (v4.0核心)"""
        # N+2→N级别传导，这里简化处理
        # 实际使用需要N+2级别数据传入
        result = {}
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
                tol = 2 if name in ['5F', '15F'] else 0
                result[name] = analyze_main_trend_segment(df, name, tolerance=tol)
        return result
    
    def get_x_segment_v40(self):
        """X段判定 v4.0 (3条件)"""
        # 需要复合结构上下文，简化版
        return self.get_x_segment()
    
    def get_segment_structure(self):
        """段数结构分析 (v4.0)"""
        result = {}
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
                strokes = find_strokes(df)
                result[name] = analyze_segment_structure(strokes, name)
        return result
    ...
    # v4.0 新增方法
    def get_segment_decomposition(self):
        """段数结构分解 (v4.0)"""
        result = {}
        for name, df in self._get_level_dict().items():
            if len(df) >= 55:
                result[name] = analyze_segment_decomposition(df, name)
        return result
    
    def get_biweek_entity(self):
        """双周K线实体分析 (v4.0)"""
        if hasattr(self, 'df_biweek') and self.df_biweek is not None:
            return analyze_biweek_entity(self.df_biweek)
        return {'usable': False}
    
    def get_watershed_120f(self):
        """120F55线战略分水岭 (v4.0)"""
        if hasattr(self, 'df_120f') and hasattr(self, 'df_daily'):
            if self.df_120f is not None and self.df_daily is not None:
                return analyze_120f_watershed(self.df_120f, self.df_daily)
        return {'usable': False}
    
    def get_dual_day_weakness_lift(self):
        """双日极弱解除条件 (v4.0经验规则)"""
        if hasattr(self, 'df_30m') and hasattr(self, 'df_biday'):
            if self.df_30m is not None and self.df_biday is not None:
                return check_dual_day_weakness_lift(self.df_30m, self.df_biday)
        return {'usable': False}
...
def main():
    print("="*60)
    print("缠论分析系统 v4.0 - 完整版")
    print("核心升级: 主涨段/MACD稳定性/X段3条件/分型笔线段")
    print("="*60)
    ...
    try:
        ...
        analyzer = ChanAnalysisV40(df_1m=df_1m, df_5m=df_5m, df_daily=df_daily)
...
def analyze_segment_decomposition(df_level, level_name='30F'):
    """
    段数结构分解 (v4.0)
    ...
    high_max_idx = recent['High'].idxmax()
    low_min = recent['Low'].min()
    low_min_idx = recent['Low'].idxmin()
    
    start_price = low_min
    start_idx = low_min_idx
    high_price = high_max
    high_idx = high_max_idx
    
    if start_idx < high_idx:
        segments = 1
        current = '上涨段进行中/回踩中'
    else:
        segments = 0
    ...
    is_reset = False
    if price < high_price and price > ma55 and macd < 0:
        is_reset = True
        current = 'X段回踩（可能重置上涨结构）'
    
    if price > ma55 and macd > 0:
        target = f'突破前高 {high_price:.2f}'
    elif price < ma55:
        target = f'测试MA55 {ma55:.2f}'
    else:
        target = '观察方向'
    ...
    return {
        ...
        'target': target,
        'is_reset': is_reset,
        'description': f'{level_name}: 起点{start_price:.2f}→高点{high_price:.2f} | {current} | 目标:{target}'
    }
...
def analyze_biweek_entity(df_biweek):
    """
    双周K线实体分析 (v4.0)
    ...
    entity_direction = '阳线' if close_p > open_p else '阴线'
    entity_center = (close_p + open_p) / 2
    
    is_entity_above_mid = entity_center > mid
    is_close_above_mid = close_p > mid
    
    if is_entity_above_mid and is_close_above_mid:
        entity_status = '实体+收盘均在中轨之上 → 多头力量有效'
        bias = '偏多'
    elif not is_entity_above_mid and not is_close_above_mid:
        entity_status = '实体+收盘均在中轨之下 → 空头力量有效'
        bias = '偏空'
    else:
        entity_status = '实体与收盘矛盾 → 方向不明'
        bias = '观望'
    
    macd_bias = '偏多' if macd > 0 else '偏空'
    contradiction = bias != macd_bias
    
    conclusion = bias
    if contradiction:
        conclusion = f'K线{bias} vs MACD{macd_bias} → 以K线实体为准:{bias}'
    
    return {
        'usable': True,
        'entity_direction': entity_direction,
        'entity_center': round(entity_center, 2),
        'mid': round(mid, 2),
        'is_entity_above_mid': is_entity_above_mid,
        'entity_status': entity_status,
        'macd_bias': macd_bias,
        'contradiction': contradiction,
        'conclusion': conclusion,
        'description': f'双周实体: {entity_direction} | 实体中心{entity_center:.2f} vs 中轨{mid:.2f} | {conclusion}'
    }


# =====================================================
# v4.0 新增: 120F55线战略分水岭
# =====================================================

def analyze_120f_watershed(df_120f, df_daily):
    """
    120F55线战略分水岭 (v4.0经验规则)
    
    双向路径:
    - 有效突破120F55线 → 双日第三段上涨确认
    - 120F55线压制 → 日线主跌段前兆 → 跌破55周线
    ...
    latest_120f = df_120f.iloc[-1]
    price = latest_120f['Close']
    ma55_120f = latest_120f['MA55']
    macd_120f = latest_120f['MACD']
    
    latest_daily = df_daily.iloc[-1]
    ma55_daily = latest_daily['MA55']
    
    diff_120f_55day = ma55_120f - ma55_daily
    
    if diff_120f_55day > 0:
        difficulty = '困难（120F55线在上，需强势放量）'
        preferred_break = '等待120F55线下行穿过55日线后再突破（时间换空间）'
    else:
        difficulty = '相对容易（120F55线已低于55日线）'
        preferred_break = '可直接突破'
    
    if price > ma55_120f and macd_120f > 0:
        status = '已突破120F55线 → 双日第三段上涨可能确认'
        path = '突破路径'
    elif price < ma55_120f and macd_120f < 0:
        status = '120F55线下方运行 → 日线主跌段风险'
        path = '压制路径'
    else:
        status = '120F55线附近胶着 → 一周内需确认方向'
        path = '观察'
    
    return {
        'usable': True,
        'price': round(price, 2),
        'ma55_120f': round(ma55_120f, 2),
        'ma55_daily': round(ma55_daily, 2),
        'diff_120f_55day': round(diff_120f_55day, 2),
        'difficulty': difficulty,
        'preferred_break': preferred_break,
        'status': status,
        'path': path,
        'time_frame': '一周内必须有答案',
        'description': f'120F55线={ma55_120f:.2f} vs 55日线={ma55_daily:.2f} | {status} | {difficulty}'
    }


# =====================================================
# v4.0 新增: 双日极弱解除条件（经验规则）
# =====================================================

def check_dual_day_weakness_lift(df_30f, df_dual_day):
    """
    双日极弱解除条件 (v4.0经验规则)
    
    经验: 30F级别有效突破MA55线 → 可传导至双日级别，解除极弱形态
    注意: 此规则为经验总结，需后续案例验证
    """
    if len(df_30f) < 55 or len(df_dual_day) < 55:
        return {'usable': False, 'description': '数据不足'}
    ...
    latest_dd = df_dual_day.iloc[-1]
    dif_dd = latest_dd['MACD']
    dea_dd = latest_dd['MACD_Signal']
    
    is_dual_day_weak = (dif_dd <= 0) and (dea_dd >= 0)
    
    is_30f_break = price_30f > ma55_30f + 2
    is_30f_real_break = is_30f_break and macd_30f > 0
    
    if is_dual_day_weak and is_30f_real_break:
        lift_status = '可能解除（30F有效突破，传导至双日极弱）'
        lift_prob = '中'
    elif is_dual_day_weak and is_30f_break:
        lift_status = '观察（30F突破但MACD未确认，可能假突破）'
        lift_prob = '低'
    elif is_dual_day_weak:
        lift_status = '未解除（30F未突破MA55）'
        lift_prob = 'N/A'
    else:
        lift_status = '双日非极弱状态'
        lift_prob = 'N/A'
    ...
    return {
        'usable': True,
        'is_dual_day_weak': is_dual_day_weak,
        'is_30f_break': is_30f_break,
        'is_30f_real_break': is_30f_real_break,
        'lift_status': lift_status,
        'lift_prob': lift_prob,
        'description': f'双日极弱:{is_dual_day_weak} | 30F突破:{is_30f_break} | 解除状态:{lift_status}'
    }
